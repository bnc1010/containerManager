...
func ImageList() (*[] Image, error) {
	rows, err:= Client.Query("select * from tb_image")
	defer rows.Close()
	if err!= nil{
		imageErrorLoger(err)
		return nil, err
...
func ImageUpdate(image *Image) bool {
	stmt, err := Client.Prepare("update tb_image set name=$1,describe=$2,pullname=$3,usegpu=$4,usable=$5,updatetime=$6, ports=$7 where id=$8")
	defer stmt.Close()
	if err != nil {
		imageErrorLoger(err)
		return false
	...
	ports_json, _ := utils.Array2Bytes(image.Ports)
	_, err = stmt.Exec(image.Name, image.Describe, image.PullName, image.UseGPU, image.Usable, image.UpdateTime, ports_json, image.Id)
	if err != nil {
		imageErrorLoger(err)
		return false
	}
